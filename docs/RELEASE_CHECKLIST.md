...
Для `thermostat` отдельно проверяются прямое соответствие logical address и
Slave ID, чтение FC03 без смещения адресов и запись FC06. Проверьте Power,
Cool/Heat, Low/Medium/High/Auto, целую Temp и SetTemp `16`, `21`, `34 °C`.
Команда `21 °C` должна записать raw `210`, а factual SetTemp должен измениться
только после FC03 read-back регистра `0x2065`.

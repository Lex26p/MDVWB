...
> Документ описывает текущие статические веб-приложения MDVWB 1.3.0.
...
Редактор шины содержит поле `Период опроса, мс`. Для MDV допустимы
`150..60000`, для Modbus RTU — `150..60000`; defaults равны `150` и `300`
соответственно. Это настройка конкретной шины. Её сохранение перезапускает
только изменённый service instance.
...
Повторная доставка retained `/mdvwb/config` с теми же revision и содержимым
не считается внешним изменением и не затрагивает локальный draft. Предупреждение
о новой конфигурации появляется только когда полученная server configuration
действительно отличается от последней принятой браузером версии.
...
При ручном редактировании можно указать `0`. Начальный номер нового placement
совпадает с его адресом в настройке шины. Если этот номер уже занят устройством
другой шины, выбирается первое свободное значение `0..200`.
...
Для `Power`, `Mode`, `Speed` и `SetTemp` страница дополнительно использует
retained `/mdvwb/config` и `/mdvwb/modbus/profiles`. MDV bus сохраняет полный
набор этих команд. Для `modbus_rtu` поле `supported` показывает semantic
...
Control включается в group panel, если его поддерживает хотя бы одно выбранное
устройство. При построении плана browser пропускает этот control отдельно для
каждой Modbus target, для которой каталог не разрешает фактическую запись;
поддерживаемые targets продолжают выполняться.
...
- control без эффективной writable capability для конкретного устройства;
...
Если optional factual value (`Mode`, `Speed`, `SetTemp`, `Temp`, `Blinds` или
`Blok`) становится недоступным, driver публикует пустой retained payload. Broker
удаляет старое значение, а открытая страница сбрасывает его в «—» вместо показа
устаревшего факта.
...
После выбора targets редактор вычисляет пересечение эффективных capabilities
всех выбранных устройств. Новую неподдерживаемую action отметить нельзя. Если
сохранённый draft уже содержит action, которую одна из targets не поддерживает,
save и manual run заблокированы, пока action или target не исправлены.
Scheduler независимо проверяет профиль и текущую runtime write capability перед
первой MQTT-командой.

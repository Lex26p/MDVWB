...
## Управление сервисом из веб-интерфейса Wiren Board

Файл `deploy/mdvwb-service-control.js` создаёт отдельное виртуальное устройство `MDVWB-Service-1` с кнопками запуска, остановки, перезапуска и обновления статуса. Результат выполнения фиксированных команд `systemctl` и текущее состояние `mdvwb.service` отображаются в текстовых контролах.

Для установки только этого блока на уже настроенный контроллер скопируйте через WinSCP файлы `deploy/mdvwb-service-control.js` и `deploy/install_service_control.sh` в одну папку и выполните от root:

```sh
chmod +x install_service_control.sh
./install_service_control.sh
```

Скрипт не принимает произвольные shell-команды и управляет только `mdvwb.service`.
